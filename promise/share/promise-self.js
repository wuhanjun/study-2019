...
class Promise {
  ...
  resolvePromise (promise2, x, resolve, reject) {
    ...
    if (x !== null && (typeof x === 'object' || typeof x === 'function')) {
      // 这个y也就是then方法中创造的promise实例resolven 的值。
      ...
      let isCalled = false
      // 需要isCalled的原因是这个promise可能是别人实现的，别人的promise不标准的话，可能状态可以从pending-resolve，resolve再到reject。这就很乱了。所以在这个函数里，我们需要严谨一些。
      try {
        ...
        // 因为如果是别人的then，每次用x.then这样的方式调用，都会通过getter取值，就会造成前后值不一样的情况。
